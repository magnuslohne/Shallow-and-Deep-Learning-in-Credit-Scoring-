...
set.seed(123)
rfmod1 <- randomForest(as.factor(Default_next_12_months) ~ ., data = train_set,
...
rf_prob_pred1 <- predict(rfmod1, newdata=validation_set, "prob")[1:nrow(validation_set)]
...
rfmod2 <- randomForest(as.factor(Default_next_12_months) ~ ., data = train_set,
...
rf_prob_pred2 <- predict(rfmod2, newdata=validation_set, "prob")[1:nrow(validation_set)]
...
rfmod3 <- randomForest(as.factor(Default_next_12_months) ~ ., data = train_set,
...
rf_prob_pred3 <- predict(rfmod3, newdata=validation_set, "prob")[1:nrow(validation_set)]
...
set.seed(123)
rfmod4 <- randomForest(as.factor(Default_next_12_months) ~ ., data = train_set,
mtry = 6, importance = TRUE, do.trace = 100, n_tree=100, classwt = class_weights)
...
rf_prob_pred4 <- predict(rfmod4, newdata=validation_set, "prob")[1:nrow(validation_set)]
...
```{r}
test_prob_pred_RF <- predict(rfmod1, newdata=test_set, "prob")[1:nrow(test_set)]
...
test_RF_class_pred <- ifelse(test_prob_pred_RF > as.numeric(optimal_threshold_RF)[1], 1, 0)

confusion_matrix_RF <- cm_fu(test_RF_class_pred, actual_class_test)
...
```{r}
var_imp_RF <- as.data.frame(randomForest::importance(rfmod1))[,c(3,4)]
